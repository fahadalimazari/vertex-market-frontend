...
import { FiX, FiStar, FiImage, FiVideo, FiTrash2, FiCheckCircle } from 'react-icons/fi';
import { useReviews } from '../../context/ReviewContext';
import { useDashboard } from '../../context/Dashboard/DashboardContext';

const EditReviewModal = ({ review, onClose }) => {
  const { editReview, isLoading } = useReviews();
  ...
  useEffect(() => {
    if (review) {
      ...
      setRecommendProduct(review.recommendProduct !== false);
    }
  }, [review]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto py-10">
      <div className="bg-white rounded-2xl w-full max-w-2xl relative shadow-xl my-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mb-6">
            <label className="block text-sm font-bold text-gray-900 mb-2">Media</label>
            <div className="flex flex-wrap gap-4">
              <button
                type="button"
                onClick={handleImageUpload}
                disabled={images.length >= 5}
                className={`w-20 h-20 rounded-xl border-2 border-dashed flex flex-col items-center justify-center gap-1 transition-colors ${images.length >= 5 ? 'border-gray-200 text-gray-300 cursor-not-allowed' : 'border-gray-300 text-gray-500 hover:border-[#ff6a00]'}`}
              >
                <FiImage size={24} />
              ...
              {!video && (
                <button
                  ...
                  className="w-20 h-20 rounded-xl border-2 border-dashed border-gray-300 text-gray-500 hover:border-[#ff6a00] flex flex-col items-center justify-center gap-1"
                >
                  <FiVideo size={24} />
              ...
              {video && (
                <div className="w-20 h-20 rounded-xl relative group overflow-hidden border border-gray-200 bg-black">
                  <img src={video.thumbnail} alt="video" className="w-full h-full object-cover opacity-70" />
                  <button 
                    type="button"
          ...
          <div className="space-y-4 border-t border-gray-100 pt-6 mb-6">
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-gray-700 font-medium">I recommend this product</span>
            </label>
          ...
          <div className="flex gap-4">
            ...
            <button 
              ...
              className="flex-1 py-3.5 px-6 rounded-xl bg-[#ff6a00] text-white font-bold hover:bg-[#e65c00] transition-colors disabled:opacity-50"
            >
